...
export default function App() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if(search) query = query.or(`title.ilike.%${search}%,company_name.ilike.%${search}%,skills_tags.cs.{${search}}`);

      const {data,error} = await query;
      if(error) throw error;

      // If no data in DB, fetch from Adzuna API (via our serverless function)
