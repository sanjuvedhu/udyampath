      ...
      {featuredJobs.length > 0 && (
        <div style={{padding:"40px 16px",maxWidth:1200,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:8}}>
            ...
            <div onClick={()=>document.getElementById("jobs-grid")?.scrollIntoView({behavior:"smooth"})} style={{fontSize:13,color:C2.sky,cursor:"pointer",fontWeight:700}}>View all jobs →</div>
          </div>
...
export default function App() {
  ...
  const liveJobs  = jobs.filter(j=>!j.filled_seats||(j.filled_seats<(j.total_seats||10))).filter(j=>{
    if(skillFilter && !((j.title||"")+" "+(j.description||"")+" "+(j.category||"")).toLowerCase().includes(skillFilter.toLowerCase())) return false;
          ...
          <div style={{maxWidth:1400,margin:"0 auto",padding:"0 12px",display:"flex",alignItems:"center",height:56,gap:8,width:"100%",boxSizing:"border-box"}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
              ...
              <button onClick={()=>setShowPricing(true)} className="btn"
                style={{padding:"8px 12px",borderRadius:12,background:"linear-gradient(135deg,#FFB700,#FF6B00)",color:"#000",border:"none",fontWeight:900,fontSize:13,cursor:"pointer",flexShrink:0,zIndex:10}}>
                💎
              </button>
